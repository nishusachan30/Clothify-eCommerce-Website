...
if (isset($_POST['send_otp'])) {
    ...
    if (empty($email)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $update_otp_query = "UPDATE admins SET otp_code = ?, otp_timestamp = ? WHERE admin_email = ?";
...
if (isset($_POST['reset_password'])) {
    ...
    if (!$email) {
        ...
        $query = "SELECT otp_code, otp_timestamp FROM admins WHERE admin_email = ?";
        $stmt = $con->prepare($query);
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            ...
            if (trim($data['otp_code']) === $entered_otp && $time_difference <= 300) {
                if ($new_password !== $confirm_password) {
                    ...
                    $update_pass_query = "UPDATE admins SET admin_pass = ?, otp_code = NULL, otp_timestamp = NULL WHERE admin_email = ?";
