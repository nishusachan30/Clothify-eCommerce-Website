<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Meta Tags -->
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="content-type" content="text/html; charset=UTF-8" />
    

    <!-- Page Title -->
    <title>Clothify Store - Men | Women | Kids |Accessories</title>

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">

    <!-- jQuery Library -->
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

    <!-- Custom Stylesheets -->
    <link rel="stylesheet" href="styles/style.css">
    <link rel="stylesheet" href="styles/bootstrap.min.css">
    <link rel="stylesheet" href="styles/backend.css">


    <!-- Font Awesome CSS -->
    <link rel="stylesheet" href="fontawesome/css/all.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" />
    <link rel="stylesheet" href="boxicons-2.1.4/css/boxicons.min.css">

    <!-- Favicon -->
    <link rel="shortcut icon" href="../images/logo.jpeg" type="image/jpeg">

    <!-- Font Awesome Kit -->
    <script src="https://kit.fontawesome.com/b49e178814.js" crossorigin="anonymous"></script>


    <!-- Inline CSS for Product Image Styling -->
    <style>
        /* Import Google Fonts */
        @import url("https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap");

        /* Styling for product images */
        .product img {
            width: 100%; /* Ensures images take up full width of the container */
            height: 250px; /* Sets a fixed height for uniform appearance */
        }
    </style>
</head>
